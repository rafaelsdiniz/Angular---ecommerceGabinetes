...
export class ItemPedidoService {
  private apiUrl = 'http://localhost:8080/itempedido'; // ou /itempedidos, verifique no backend
  ...
  listarTodos(): Observable<ItemPedido[]> {
    return this.http.get<ItemPedido[]>(this.apiUrl, this.httpOptions);
  }

  criar(item: ItemPedido): Observable<ItemPedido> {
    return this.http.post<ItemPedido>(this.apiUrl, item, this.httpOptions);
  }

  buscarPorId(id: number): Observable<ItemPedido> {
    return this.http.get<ItemPedido>(`${this.apiUrl}/${id}`, this.httpOptions);
  }

  atualizar(id: number, item: ItemPedido): Observable<ItemPedido> {
    return this.http.put<ItemPedido>(`${this.apiUrl}/${id}`, item, this.httpOptions);
  }
  ...
  listarPorClienteLogado(): Observable<ItemPedido[]> {
    return this.http.get<ItemPedido[]>(`${this.apiUrl}/meus-itens`, this.httpOptions);
  }
}
